...
class Product:
    ...
    def insert_sql_query_product(self):
        ...
        return ('INSERT IGNORE INTO product '
                '(link, name, nutriscore, category_name) '
                'VALUES ("{}", "{}", "{}", "{}");'.format(
    ...
    def insert_sql_query_prod_store_relation(self, store):
        """returns the SQL script to insert a product and a store
        in the product_store_relation table"""

        return ('INSERT IGNORE INTO product_store_relation '
                '(product_link, store_name) VALUES '
                '("{}", "{}");'.format(self.link, store))
    ...
    def select_sql_query_info(self):
        """returns the SQL script to get product information
        from the product table"""

        return ('SELECT name, nutriscore FROM product '
                'WHERE link = "{}";'.format(self.link))

    @property
    def select_sql_query_stores(self):
        """returns the SQL script to get stores related to a product
        in the product_store_relation"""

        return ('SELECT store_name FROM product_store_relation '
                'WHERE product_link = "{}";'.format(self.link))

    @property
    def select_sql_query_substitute(self):
        """returns the SQL script to get healthy products
        related to a product in the products_users_relation"""
        return ('SELECT good_product_link FROM products_users_relation '
                'WHERE bad_product_link = "{}";'.format(self.link))
...
class Category:
    ...
    def insert_sql_query(self):
        ...
        return ('INSERT IGNORE INTO category (name) '
                'VALUES ("{}")'.format(self.name))

    @property
    def select_sql_query_products(self):
        """returns the SQL script to get products in the product table
        for a defined category"""

        return ('SELECT name, link, nutriscore FROM product '
                'WHERE category_name = "{}";'.format(self.name))

    @property
    def select_sql_query_better_products(self):
        """returns the SQL script to get products in the product table
        for a defined category, ordered by nutriscore"""

        return ('SELECT * FROM product WHERE category_name = "{}" '
                'ORDER BY nutriscore ASC;'.format(self.name))
...
class User:
    ...
    def insert_sql_query(self):
        ...
        return ('INSERT IGNORE INTO user (email_address) VALUES '
    ...
    def insert_sql_query_prod_user_relation(self, good_product, bad_product):
        """returns the SQL script to insert a product ,
        substitute and user in the products_users_relation table"""
    ...
    def select_sql_query_prod(self):
        """returns the SQL script to get unhealthy products in the
        products_users_relation table for a defined user"""

        return ('SELECT DISTINCT bad_product_link FROM products_users_relation'
                ' WHERE user_email_address = "{}";'.format(self.email_address))
